...
class OrchestratorClient:
    ...
    # ------------------------------------------------------------------
    # S3 upload (direct from worker — orchestrator never sees PDF bytes)
    # ------------------------------------------------------------------

    async def upload_pdf_to_s3(self, presigned_url: str, pdf_bytes: bytes) -> bool:
        """
        Upload PDF bytes directly to S3 using a presigned PUT URL.

        This uses a separate httpx client with no auth headers and a longer
        timeout, since S3 is a different host from the orchestrator.
        """
        async with httpx.AsyncClient(timeout=self.s3_timeout) as s3_client:
            for attempt in range(self.max_retries + 1):
                try:
    ...
    async def confirm_uploads(
        self, judge_name: str, uploads: List[Dict[str, Any]]
    ) -> None:
        """
        Confirm that PDFs were successfully uploaded to S3.

        Optional endpoint — if the orchestrator doesn't implement it, this
        is a no-op failure that the worker can safely ignore.
        """
        try:
            await self._retry_request(
                "POST",
                f"/api/jobs/{judge_name}/uploads/complete",
                json={"uploads": uploads},
            )
        except httpx.HTTPStatusError as e:
            # If the endpoint doesn't exist (404), just log and move on
